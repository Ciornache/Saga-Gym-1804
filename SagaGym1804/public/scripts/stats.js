...
async function loadFavorites() {
  ...
  data.favTypes.forEach(t => {
    const pct = data.favCount
      ? Math.round(t.value / data.favCount * 100)
      : 0;
  ...
  ol.innerHTML = data.likedExercises
    .map(name => `<li>${name}</li>`)
    .join('');
  ...
  legend.innerHTML = data.muscleGroups.map((m,i) => `
    <span>
      <i class="fas fa-circle" style="color:${['#e63946','#2a9d8f','#f06292','#6a4c93'][i]}"></i>
      ${m.label} ${Math.round(m.value / values.reduce((a,b)=>a+b,0)*100)}%
    </span>
  `).join('');
...
async function loadActivity() {
  ...
  document.querySelector('.total-workouts .value').textContent   = a.totalWorkouts;
  document.querySelector('.total-sets .value').textContent       = a.totalSets;
  document.querySelector('.total-exercises .value').textContent  = a.totalExercises;
  document.querySelector('.total-weight .value').textContent     = a.totalWeight + ' kg';
  ...
  tbody.innerHTML = `
    <tr>
      ...
      <td class="${thisWeek>=lastWeek?'positive':'negative'}">
        ${lastWeek ? Math.round((thisWeek - lastWeek)/lastWeek*100) + '%' : '—'}
      </td>
    </tr>
    <tr>
      <td>km Ran</td>
      ...
      <td class="${kmThisWeek>=kmLastWeek?'positive':'negative'}">
        ${kmLastWeek ? Math.round((kmThisWeek - kmLastWeek)/kmLastWeek*100) + '%' : '—'}
      </td>
    </tr>
  `;
